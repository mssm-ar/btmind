...
class OpenRouterService(BaseGenerationService):
    ...
    def __init__(self, config: Any = None):
        ...
        # Running total of OpenRouter spend for this miner process (USD).
        # Reset on miner restart; intended for at-a-glance cost visibility in logs.
    ...
    def _log_cost(self, modality: str, model: str, usage: Optional[Dict[str, Any]]) -> Optional[float]:
        """Log the USD cost OpenRouter reports for a generation and update the running total.
        
        `usage` is the ``usage`` object OpenRouter includes in chat-completion responses
        (image gen) and completed video-job responses. Returns the cost if present.
        """
        cost = (usage or {}).get("cost")
        if cost is None:
            bt.logging.warning(
                f"OpenRouter {modality} response for model={model} did not include usage.cost"
            )
            return None
        
        self.total_cost_usd += cost
        bt.logging.success(
            f"💵 OpenRouter {modality} generation cost: ${cost:.4f} "
            f"(model={model}, session total: ${self.total_cost_usd:.4f})"
        )
        return cost
    ...
    async def _generate_image(self, prompt: str, model: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Log cost before any further parsing, so we still capture spend
            # even if downstream image-extraction logic fails.
            cost = self._log_cost("image", model, result.get("usage"))
            ...
            if 'message' not in choice or 'images' not in choice['message']:
                ...
                bt.logging.error(
                    f"OpenRouter response missing images in message "
                    f"(finish_reason={choice.get('finish_reason')}, "
                    f"message_keys={list(message.keys())}, "
                    f"content={str(message.get('content'))[:300]!r})"
                )
    ...
    def _generate_video(
        ...
        # Phase 2: Poll
        try:
            job_result = self._poll_video_job(job_id, headers, poll_interval, poll_timeout, start_time)
        finally:
            oc(None)
        ...
        return {
            ...
            "metadata": {
                ...
                "cost_usd": (job_result.get("usage") or {}).get("cost"),
            },
        }
    ...
    def _poll_video_job(
        ...
        while True:
            ...
            if status == "completed":
                self._log_cost("video", job.get("model", "unknown"), job.get("usage"))
                return job
