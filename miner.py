...
class GenerativeMiner(BaseNeuron):
    ...
    async def _call_service_process(self, service, task):
        ...
        try:
            ...
            # Route by task modality first: some services (e.g. OpenRouterService) split
            # image/video handling across an async image-only `process()` and a sync
            # video-only `process_with_checkpoint()`, rather than dispatching on modality
            # inside a single method like other services do. Checking
            # `iscoroutinefunction(service.process)` alone would always pick the
            # image-only async path for such services, even for video tasks.
            if task.modality == "video":
                result = service.process_with_checkpoint(task, cb)
            elif inspect.iscoroutinefunction(service.process):
